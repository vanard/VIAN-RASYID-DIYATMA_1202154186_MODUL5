...
public class TodoHelper extends SQLiteOpenHelper{
    ...
    public boolean inputdata(TodoItem list) {
    ...
    public void readdata(ArrayList<TodoItem> daftar){
        Cursor cursor = this.getReadableDatabase().rawQuery("select todo, description, priority from "+nama_tabel, null);
        while (cursor.moveToNext()){
            daftar.add(new TodoItem(cursor.getString(0), cursor.getString(1), cursor.getString(2)));
        }
    }
}
